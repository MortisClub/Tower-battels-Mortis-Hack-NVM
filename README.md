...
- **Visual Boost (Lightning Overload)**  
  Визуально разгоняет все эффекты, связанные с молниями/шок‑баффами — интерфейс начинает явно показывать «перегруженные» значения баффов (`+9999999%` и т.п.), а сами эффекты в мире становятся плотнее и заметнее.  
  За счёт этого проще контролировать, какие зоны реально находятся под действием буста (диапазон DJ/Commander и других аур), когда именно включены/обновлены усиления, и где сейчас идёт максимальный ДПС по линии.  

- **Управление GUI**  
  - **K** — скрыть / показать весь Rayfield‑интерфейс.  
